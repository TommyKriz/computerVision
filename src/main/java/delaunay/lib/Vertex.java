...
/**
 * Points in Euclidean space, implemented as double[]. Includes simple geometric operations.
 * Uses matrices; a matrix is represented as an array of Pnts. Uses simplices; a simplex is 
 * represented as an array of Pnts.
 * 
 * @author Paul Chew (2005, see COPYRIGHT.TXT)
 * @version 2016/03/08 W. Burger (WB): modified, changed most methods to protected.
 * 		Renamed this class to 'Vertex' to avoid confusion.
 */
public class Vertex {
    ...
    public Vertex (double... coords) {
        ...
        this.coordinates = coords.clone();	// changed (WB)
    }
    ...
    /**
     * @return the specified coordinate of this vertex.
     ...
     */
    protected int dimCheck (Vertex p) {
     ...
     */
    protected Vertex extend (double... coords) {
     ...
     */
    protected Vertex subtract (Vertex p) {
        int len = dimCheck(p);
     ...
     */
    protected double angle (Vertex p) {
     ...
     */
    protected Vertex bisector (Vertex point) {
        dimCheck(point);
        Vertex diff = this.subtract(point);
        Vertex sum = this.add(point);
     ...
     */
    protected int[] relation (Vertex[] simplex) {
     ...
     */
    protected Vertex isOutside (Vertex[] simplex) {
     ...
     */
    protected Vertex isOn (Vertex[] simplex) {
        int[] result = this.relation(simplex);
        Vertex witness = null;
        for (int i = 0; i < result.length; i++) {
     ...
     */
    protected boolean isInside (Vertex[] simplex) {
        int[] result = this.relation(simplex);
     ...
     */
    protected int vsCircumcircle (Vertex[] simplex) {
        Vertex[] matrix = new Vertex[simplex.length + 1];
        for (int i = 0; i < simplex.length; i++)
            matrix[i] = simplex[i].extend(1, simplex[i].dot(simplex[i]));
        matrix[simplex.length] = this.extend(1, this.dot(this));
        double d = Vertex.determinant(matrix);
        int result = (d < 0)? -1 : ((d > 0)? +1 : 0);
        if (Vertex.content(simplex) < 0) result = - result;
        return result;
    }
	 ...
	 */
	public static String toString (Vertex[] matrix) {
	    StringBuilder buf = new StringBuilder("{");
	    for (Vertex row: matrix) buf.append(" " + row);
	    buf.append(" }");
	    return buf.toString();
	}
	...
	/**
		 ...
		 */
	protected static Vertex circumcenter (Vertex[] simplex) {
		    int dim = simplex[0].dimension();
		    if (simplex.length - 1 != dim)
		        throw new IllegalArgumentException("Dimension mismatch");
		    Vertex[] matrix = new Vertex[dim];
		    for (int i = 0; i < dim; i++)
		        matrix[i] = simplex[i].bisector(simplex[i+1]);
		    Vertex hCenter = cross(matrix);      // Center in homogeneous coordinates
	//	    double last = hCenter.coordinates[dim];
		    double last = hCenter.coord(dim);
		    double[] result = new double[dim];
		    for (int i = 0; i < dim; i++) {
	//	    	result[i] = hCenter.coordinates[i] / last;
		    	result[i] = hCenter.coord(i) / last;
		    }
		    return new Vertex(result);
		}
	...
	/**
		 ...
		 */
		protected static double determinant(Vertex[] matrix, int row, boolean[] columns){
	 ...
	 */
	protected static double determinant (Vertex[] matrix) {
	    ...
	    try {return Vertex.determinant(matrix, 0, columns);}
	 ...
	 */
	protected static Vertex cross (Vertex[] matrix) {
	    ...
	    try {
	        for (int i = 0; i < len; i++) {
	            columns[i] = false;
	            result[i] = sign * Vertex.determinant(matrix, 0, columns);
	            columns[i] = true;
	            sign = -sign;
	        }
	    } catch (ArrayIndexOutOfBoundsException e) {
	 ...
	 */
	protected static double content (Vertex[] simplex) {
	    Vertex[] matrix = new Vertex[simplex.length];
	    for (int i = 0; i < matrix.length; i++)
	        matrix[i] = simplex[i].extend(1);
	    int fact = 1;
	    for (int i = 1; i < matrix.length; i++) fact = fact*i;
	    return Vertex.determinant(matrix) / fact;
	}
	
	// -------- WB added -----------------------------------------------------
	
	/**
	 * Converts this {@literal Vertex} to a {@literal Point2D} instance.
	 * 
	 * @return a new 2D point
	 */
//	public Point2D toPoint2D() {
//		return new Point2D.Double(this.coord(0), this.coord(1));
//	}
	
	/**
	 * Constructor. Creates a {@literal Vertex} from a {@literal Point2D}
	 * instance.
	 * 
	 * @param p a 2D point (with floating-point coordinates)
	 */
...
//        System.out.println(Pnt.determinant(matrix1) + " " + Pnt.determinant(matrix2));
