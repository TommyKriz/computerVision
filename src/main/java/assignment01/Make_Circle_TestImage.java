...
/**
 * This ImageJ plugin creates a binary image containing random points on a
 * specified circle plus additional random (noise) points uniformly distributed
 * in the image. Marked points have values I(u,v) = 255, background pixels have
 * zero values. The display is inverted, i.e., background pixels are shown
 * white.
 * 
 * @author W. Burger
 * @version 2018-08-27
...
public class Make_Circle_TestImage implements PlugIn {
	...
	private static double xySigma = 0.5; // standard deviation of Gaussian
											// position noise
	...
	public void run(String arg0) {
		...
		for (int i = 0; i < onCirclePts; i++) {
			double phi = rg.nextDouble() * 2 * Math.PI; // uniformly distributed
														// angle 0,...,2pi
			double dx = xySigma * rg.nextGaussian(); // position disturbed by
														// Gaussian noise
