...
public class SiftMatchingMontage {
  ...
  public SiftMatchingMontage(ImageProcessor image1, ImageProcessor image2,
      List<SiftMatch> matches) {

    showMontage(image1, image2, matches);
  }
  ...
  private ProjectiveMapping calcProjection(List<SiftMatch> matches) {
    ...
    for (SiftMatch m : matches) {
      featureCoordinatesInImage1
          .add(new Point2D.Double(m.getDescriptor1().getX(), m.getDescriptor1().getY()));
      featureCoordinatesInImage2
          .add(new Point2D.Double(m.getDescriptor2().getX(), m.getDescriptor2().getY()));
    }

    return new ProjectiveMapping(featureCoordinatesInImage2.toArray(new Point2D[0]),
        featureCoordinatesInImage1.toArray(new Point2D[0]), true);
  }



  private void showMontage(ImageProcessor image1, ImageProcessor image2, List<SiftMatch> matches) {


    calcProjection(matches).applyTo(image2, InterpolationMethod.Bicubic);

    // get

    // 4 punkte
    // min max
    // um minW minH verscheiebne


    // image1.copyBits(image2, 0, 0, Blitter.AVERAGE);
    ...
    montage.copyBits(image2, 0, 0, Blitter.AVERAGE);
    // montage.insert(image2, 0, 0);

    ImagePlus montageIm = new ImagePlus("-matches", montage);

    //
    // Overlay oly = new Overlay();
    // oly.add(makeStraightLine(w, 0, w, h, Color.YELLOW)); // vertical separator
    // int xoffset = w;
    //
    //
  ...
  private void drawSiftMarkers(List<SiftMatch> matches, Overlay oly, int xoffset) {
  ...
  private ShapeRoi makeConnectingLine(SiftDescriptor f1, SiftDescriptor f2, double xo, double yo,
      Color col) {
    double x1 = f1.getX();
    double y1 = f1.getY();
